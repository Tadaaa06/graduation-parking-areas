...
map.getView().fit([3915344.179510, -1785071.432680, 3916345.312516, -1784369.203791], map.getSize());
...
////controls container
    ...
    northArrowElement.id = 'north-arrow';
    northArrowElement.style.position = 'absolute';
    northArrowElement.style.top = '60px';
    northArrowElement.style.right = '10px';
    northArrowElement.style.width = '50px';
    northArrowElement.style.height = '50px';
    northArrowElement.style.zIndex = '100';
    northArrowElement.innerHTML = '<svg viewBox="0 0 50 50" style="width: 100%; height: 100%;">' +
        '<polygon points="25,5 35,40 25,35 15,40" fill="#333" stroke="#333" stroke-width="1"/>' +
        '<text x="25" y="48" text-anchor="middle" font-size="12" font-weight="bold" fill="#333">N</text>' +
        '</svg>';
    
    document.getElementById('map').appendChild(northArrowElement);
    
    // Update north arrow rotation based on map view rotation
    map.getView().on('change:rotation', function() {
        var rotation = map.getView().getRotation();
        var northArrow = document.getElementById('north-arrow');
        if (northArrow) {
            northArrow.style.transform = 'rotate(' + (-rotation * 180 / Math.PI) + 'deg)';
        }
    });
